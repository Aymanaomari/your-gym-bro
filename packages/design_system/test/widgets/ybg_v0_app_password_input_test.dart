...
import 'package:design_system/widgets/password_input/ybg_v0_app_password_input.dart';
...
void main() {
  ...
  testWidgets('suffix icon color reacts to focus changes', (tester) async {
    ...
    Icon idleIcon = tester.widget(find.byIcon(Icons.visibility_outlined));
    expect(idleIcon.color, equals(theme.ygbColors.primary50));

    await tester.tap(find.byType(TextField));
    await tester.pumpAndSettle();

    Icon focusedIcon = tester.widget(find.byIcon(Icons.visibility_outlined));
    expect(focusedIcon.color, equals(theme.ygbColors.primary500));
  });
  ...
  testWidgets('error state overrides focus styling', (tester) async {
    ...
    final suffixIcon = tester.widget<Icon>(
